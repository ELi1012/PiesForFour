...
if (mouse_check_button_pressed(mb_left)) {
	//show_debug_message("------------");
	var ddd = 0; repeat (array_length_1d(spawn_set)) {
...
if (intint != array_length_1d(spawn_set) - 1) {
...
if (toggle_spawn) { //keyboard_check_pressed(ord("U"))
	repeat (spawn_count) { //relies on pp being set
		...
		cluster_iinst = instance_create_layer(-3, -3, "Instances", obj_cluster);
		//show_debug_message("cluster " + string(cluster_count) + " created");
		//show_debug_message(string(lined_up) + " game lined up");
	
		with (cluster_iinst) {
			cluster = other.cluster_count; //cluster id
			cluster_number = other.cluster_number;
		
			ds_clustomers = ds_grid_create(4, other.cluster_number);
			ds_grid_clear(ds_clustomers, -1);
		}
		...
		repeat (cluster_number) {
			...
			with (inst) {
				...
				var pie_entire = other.whole_pie;
				
				cluster = other.cluster_count; //which cluster
				cluster_id = other.cluster_iinst; //cluster instance id
				...
				cluster_id.ds_clustomers[# 0, i] = id;
				cluster_id.ds_clustomers[# 1, i] = x;
				cluster_id.ds_clustomers[# 2, i] = y;
				cluster_id.ds_clustomers[# 3, i] = false; //whether customer is seated or not
			
			}
			inc += 1;
		}
	
		var lead = cluster_iinst.ds_clustomers[# 0, 0];
